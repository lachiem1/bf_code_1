...
--  storyboard.gotoScene("mediumoverlay", options)
--  return true;
...
function onTap(event)
  ...
  storyboard.gotoScene("mediumoverlay")
  end
end
...
triangle.y = display.contentCenterY - 15
...
local greencircle = display.newCircle(0, 0, 10)
  ...
  greencircle.x = 20
  greencircle.y = display.contentCenterY + 95
...
local bluecircle = display.newCircle(0, 0, 10)
  ...
  bluecircle.x = 20
  bluecircle.y = display.contentCenterY + 120
  group:insert(bluecircle)
  scrollView:insert(bluecircle)

local greenrect = display.newRect(0, 0, 5, 60)
  greenrect:setFillColor(0,255,0)
  greenrect.x = display.contentWidth - 50
  greenrect.y = display.contentCenterY + 110
  group:insert(greenrect)
  scrollView:insert(greenrect)

local bluerect = display.newRect(0, 0, 5, 60)
  bluerect:setFillColor(0,0,255)
  bluerect.x = display.contentWidth - 35
  bluerect.y = display.contentCenterY + 110
...
local myText = [[Easy starts off nice and simple. Move triangle left and right by dragging it. 




Try and hit the bubbles into the zone with the matching colour. This one starts off simple, there are only 2 colours. 
...
local easyTxt = display.newText("Easy",0,0,native.systemFontBold, 35)
easyTxt:setFillColor(255,255,255)
easyTxt.x = display.contentCenterX
easyTxt.y = 50
group:insert(easyTxt)
scrollView:insert(easyTxt)
...
function scene:exitScene( event )
 ...
 storyboard.removeScene("easyoverlay")
