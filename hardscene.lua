...
--  storyboard.gotoScene("rotationoverlay", options)
--  storyboard.removeScene("hardoverlay")
--  return true;
...
local redcircle = display.newCircle(0, 0, 10)
  ...
  redcircle.x = 35
  redcircle.y = display.contentCenterY + 65
  group:insert(redcircle)
  scrollView:insert(redcircle)

local whitesquare = display.newRect(0, 0, 20, 20)
  whitesquare:setFillColor(255,255,255)
  whitesquare.x = 35
  whitesquare.y = display.contentCenterY + 95
  group:insert(whitesquare)
  scrollView:insert(whitesquare)
...
local medTxt = display.newText("Hard",0,0,native.systemFontBold, 35)
